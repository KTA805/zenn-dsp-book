...
前の記事ではスペクトル減算・Wienerフィルタ・スペクトルゲーティングをRMS誤差で比較しました。結果はSS < SG < Wienerの順で残差が小さく、一見SSが最も優れた手法に見えました。

しかしRMS誤差が小さい＝音声認識精度が高い、とは限りません。スペクトル減算は音声成分ごと削ることでRMS誤差を小さくしますが、削られた音声成分は当然認識もできなくなります。

この記事では、**OpenAI Whisper で実際に文字起こしをして WER（単語誤り率）で比較**します。RMS誤差との乖離が明確になります。
...
![SNR vs. WER: Noise Reduction Methods Comparison](./images/snr_vs_wer.png)
